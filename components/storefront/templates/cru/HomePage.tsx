...
export function CruHomePage({ tenant, cfg, vedettes, slug }: Props) {
  const c = cfg.colors;
  ...
  const NODES: Record<string, React.ReactNode> = {
    ...
    hero: sec.hero.actif ? (
      <section className="grid lg:grid-cols-2 items-center gap-10" style={{ minHeight: "90vh", padding: "0 4vw" }}>
        <div>
          {sec.hero.badgeTexte && <div className="mb-5 text-xs uppercase tracking-widest" style={{ color: c.accent }}>{sec.hero.badgeTexte}</div>}
          <h1 className="italic max-w-[13ch]" style={{ fontSize: "clamp(38px,4.6vw,58px)", lineHeight: 1.14 }}>{sec.hero.titre}</h1>
          <p className="mt-6 max-w-[36ch] leading-loose" style={{ color: c.texteMuted }}>{sec.hero.sousTitre}</p>
          <Link href={ctaHref} className="inline-flex mt-8 px-8 py-4 text-[12.5px] uppercase tracking-widest transition-colors" style={{ border: `1px solid ${c.accent}`, color: c.accent }}>{sec.hero.ctaTexte}</Link>
        </div>
        <div className="relative overflow-hidden" style={{ aspectRatio: "1/1.25", background: tenant.bannerUrl ? undefined : `radial-gradient(120% 100% at 50% 20%, ${c.accentSecondaire || c.accent} 0%, ${c.surface} 60%, ${c.fond} 100%)` }}>
    ...
    confiance: sec.confiance?.actif && (sec.confiance.items?.length ?? 0) > 0 ? (
      <section className="py-3.5 overflow-hidden" style={{ borderTop: `1px solid ${c.bordure}`, borderBottom: `1px solid ${c.bordure}` }}>
        <div className="ax-cru-marquee-track flex items-center w-max text-xs uppercase tracking-widest" style={{ color: c.texteMuted }}>
          {[...sec.confiance.items, ...sec.confiance.items].map((it, i) => <span key={i} className="flex-shrink-0 px-9 whitespace-nowrap">{it.icone} {it.titre} — {it.texte}</span>)}
        </div>
        <style>{`.ax-cru-marquee-track{animation:axCruMarquee 27s linear infinite;} .ax-cru-marquee-track:hover{animation-play-state:paused;} @keyframes axCruMarquee{from{transform:translateX(0);}to{transform:translateX(-50%);}} @media(prefers-reduced-motion:reduce){.ax-cru-marquee-track{animation:none;}}`}</style>
      </section>
    ) : null,
    ...
    vedettes: sec.vedettes.actif && vedettes.length > 0 ? (
      <section className="text-center" style={{ padding: "110px 4vw 40px" }}>
        <div className="mb-12">
          <span className="block mb-3 text-xs uppercase tracking-widest" style={{ color: c.accent }}>Sélection de la cave</span>
          <h2 className="text-[36px] italic">{sec.vedettes.titre}</h2>
        </div>
        <div className="text-left"><CruProductGrid produits={vedettes} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /></div>
      </section>
    ) : null,

    collections: sec.collections.actif && collections.length > 0 ? (
      <section style={{ padding: "20px 4vw 40px" }}>
        <h2 className="text-[30px] italic mb-9 text-center">{sec.collections.titre}</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {collections.slice(0, 4).map((col) => (
            <Link key={col.slug} href={`/${slug}/collections/${col.slug}`} className="relative overflow-hidden flex items-end p-5" style={{ aspectRatio: "3/4", backgroundColor: c.surface }}>
              {col.imageUrl ? <img src={col.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover opacity-80" /> : <div className="absolute inset-0" style={{ background: `radial-gradient(120% 90% at 40% 20%, ${c.accentSecondaire || c.accent}55, ${c.surface} 70%)` }} />}
    ...
    about: sec.about?.actif ? (
      <section style={{ padding: "70px 4vw" }}>
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="relative overflow-hidden" style={{ aspectRatio: "4/3", background: sec.about.imageUrl ? undefined : `radial-gradient(120% 90% at 40% 20%, ${c.accentSecondaire || c.accent}55, ${c.surface} 70%)` }}>{sec.about.imageUrl && <img src={sec.about.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover" />}</div>
          <div>
            {sec.about.badgeTexte && <div className="mb-3 text-xs uppercase tracking-widest" style={{ color: c.accent }}>{sec.about.badgeTexte}</div>}
            <h3 className="text-[28px] italic mb-4">{sec.about.titre}</h3>
    ...
    promo: sec.promo.actif ? (
      <section className="text-center" style={{ padding: "20px 4vw 70px" }}>
        <div className="py-16 px-8" style={{ backgroundColor: c.surface }}>
          <h2 className="text-[28px] italic mb-4">{sec.promo.titre}</h2>
    ...
    faq: sec.faq?.actif && (sec.faq.items?.length ?? 0) > 0 ? (
      <section style={{ padding: "20px 4vw 70px" }}>
        <h2 className="text-[26px] italic mb-8 text-center">{sec.faq.titre}</h2>
    ...
    avis: sec.avis.actif && (tenant.avis?.length ?? 0) > 0 ? (
      <section style={{ padding: "20px 4vw 70px" }}>
        <h2 className="text-[26px] italic mb-9 text-center">{sec.avis.titre}</h2>
        <div className="grid sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {tenant.avis.slice(0, 6).map((a: any) => (
            <div key={a.id}>
              ...
              <p className="text-[12px] text-center" style={{ color: c.texteMuted }}>{a.client?.nom || "Client"}</p>
            </div>
          ))}
        </div>
      </section>
    ...
    newsletter: sec.newsletter.actif ? (
      <section style={{ padding: "20px 4vw 90px" }}>
        <div className="max-w-xl mx-auto text-center">
          <h2 className="text-[24px] italic mb-3">{sec.newsletter.titre}</h2>
