...
export function CruNav({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between backdrop-blur-md" style={{ padding: "24px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}>
      <Link href={`/${slug}`} className="text-[23px] italic">
        {tenant.logoUrl ? <img src={tenant.logoUrl} alt={tenant.nomBoutique} className="h-8 object-contain" /> : tenant.nomBoutique}
      </Link>
      <nav className="hidden md:flex">
        <ul className="flex gap-8 list-none text-[13.5px]">{navLinks.map((l) => <li key={l.href}><Link href={l.href}>{l.label}</Link></li>)}</ul>
      </nav>
            ...
            <div className="absolute top-full right-0 mt-3 w-72 z-50" style={{ backgroundColor: c.surface, border: `1px solid ${c.bordure}` }}>
              {resultats.map((p) => (
                <Link key={p.id} href={`/${slug}/produits/${p.id}`} onClick={() => setDropdownOuvert(false)} className="flex items-center gap-3 px-3 py-2.5" style={{ borderBottom: `1px solid ${c.bordure}` }}>
                  ...
                  <div className="min-w-0 flex-1"><p className="text-[12.5px] truncate italic">{p.nom}</p><p className="text-[11px]" style={{ color: c.accent }}>{formatMontant(p.prix, devise)}</p></div>
                </Link>
              ))}
            </div>
          )}
      ...
      {menuOuvert && (
        <div className="md:hidden absolute top-full left-0 right-0 z-40 flex flex-col gap-1 py-4 px-6" style={{ backgroundColor: c.fond, borderBottom: `1px solid ${c.bordure}` }}>
          {navLinks.map((l) => <Link key={l.href} href={l.href} onClick={() => setMenuOuvert(false)} className="py-2.5 text-sm">{l.label}</Link>)}
        </div>
      )}
    </header>
  );
