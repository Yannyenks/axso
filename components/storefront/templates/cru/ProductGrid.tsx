...
function fallbackGradient(accent: string, secondaire: string, i: number): string {
  const angles = [`radial-gradient(120% 100% at 50% 20%, ${secondaire} 0%, #28151B 60%, #1B0E12 100%)`, `radial-gradient(120% 100% at 50% 20%, #2a1f14 0%, #241a10 60%, #1B0E12 100%)`, `radial-gradient(120% 100% at 50% 20%, #1e2420 0%, #171d19 60%, #1B0E12 100%)`];
...
export function CruProductGrid({ produits, slug, devise, commissionRate, colors: c }: Props) {
      ...
      {produits.map((p, i) => {
        ...
        return (
          <Link key={p.id} href={`/${slug}/produits/${p.id}`} className="relative overflow-hidden group text-left" style={{ backgroundColor: c.fond }}>
            <div className="relative overflow-hidden" style={{ aspectRatio: "1/1.2", background: img1 ? undefined : fallbackGradient(c.accent, c.accentSecondaire || c.accent, i) }}>
            ...
            <div className="flex items-baseline justify-between py-5 px-1">
              <span className="text-[17px] italic truncate pr-2">{p.nom}</span>
              <span className="text-xs flex-shrink-0" style={{ color: c.accent }}>{formatMontant(prixAffiche, devise)}</span>
            </div>
          </Link>
        );
      })}
