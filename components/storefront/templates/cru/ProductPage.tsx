...
export function CruProductPage({ tenant, cfg, slug, produit, prixAffiche, prixCompareAffiche, produitsSimilaires }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      <header className="sticky top-0 z-40 flex items-center justify-between backdrop-blur-md" style={{ padding: "18px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}>
        <Link href={`/${slug}`} className="text-lg italic">{tenant.nomBoutique}</Link>
      ...
      <section className="grid lg:grid-cols-[90px_1fr_1fr] gap-7" style={{ padding: "50px 4vw" }}>
        <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-visible order-2 lg:order-1">
          {images.map((img, i) => (
            <button key={i} onClick={() => setActif(i)} className="flex-shrink-0 overflow-hidden" style={{ width: 90, height: 80, border: `1px solid ${i === actif ? c.accent : "transparent"}` }}>
              <img src={img} alt="" className="w-full h-full object-cover" />
            </button>
          ))}
        </div>
        <div className="relative overflow-hidden order-1 lg:order-2" style={{ aspectRatio: "1/1.2", backgroundColor: c.surface }}>
          {images[actif] ? <img src={images[actif]} alt={produit.nom} className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ background: `radial-gradient(120% 90% at 40% 20%, ${c.accentSecondaire || c.accent}55, ${c.surface} 70%)` }} />}
        </div>
        <div className="order-3 flex flex-col justify-center">
          <div className="mb-3 text-xs uppercase tracking-widest" style={{ color: c.accent }}>{produit.categorie || "Sélection"}</div>
          <h1 className="text-[32px] italic mb-4">{produit.nom}</h1>
          ...
          {Object.entries(dimensions).map(([dim, variantes]) => (
            <div key={dim} className="mb-6">
              <p className="mb-2.5 text-xs uppercase tracking-widest" style={{ color: c.texteMuted }}>{dim}</p>
              <div className="flex flex-wrap gap-2">
                {variantes.map((v: any) => (
                  <button key={v.id} onClick={() => setSelection((s) => ({ ...s, [dim]: v.id }))} disabled={v.stock === 0}
                    className="px-4 py-2 text-sm disabled:opacity-30" style={{ border: `1px solid ${selection[dim] === v.id ? c.accent : c.bordure}`, color: selection[dim] === v.id ? c.accent : c.texte }}>{v.valeur}</button>
                ))}
              </div>
            </div>
          ))}
      ...
      {(produit.avis?.length ?? 0) > 0 && (
        <section style={{ padding: "20px 4vw 70px", borderTop: `1px solid ${c.bordure}` }}>
          <h2 className="text-[24px] italic mb-8 mt-10">Avis clients</h2>
      ...
      {produitsSimilaires.length > 0 && (
        <section style={{ padding: "20px 4vw 80px", borderTop: `1px solid ${c.bordure}` }}>
          <h2 className="text-[24px] italic mb-8 mt-10">Vous aimerez aussi</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]" style={{ backgroundColor: c.bordure }}>
            {produitsSimilaires.map((p) => (
              <Link key={p.id} href={`/${slug}/produits/${p.id}`} style={{ backgroundColor: c.fond }}>
                <div className="relative overflow-hidden" style={{ aspectRatio: "1/1.2" }}>{p.images[0] ? <img src={p.images[0]} alt={p.nom} className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ backgroundColor: c.surface }} />}</div>
