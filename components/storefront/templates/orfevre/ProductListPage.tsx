...
export function OrfevreProductListPage({ tenant, cfg, produits, slug, filtres }: Props) {
  const c = cfg.colors;
    ...
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section style={{ padding: "60px 4vw 40px" }}>
        <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-6 mb-9">
          <h1 className="text-[32px] italic" style={{ fontWeight: 400 }}>Tous les produits</h1>
        ...
        {collections.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-9 text-[12px] uppercase tracking-wide">
        ...
        {produits.length > 0 ? <OrfevreProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-20 text-center" style={{ color: c.texteMuted }}>Aucun produit ne correspond à votre recherche.</p>}
      </section>
