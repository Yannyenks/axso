...
export function OrfevreCollectionPage({ tenant, cfg, collection, produits, slug }: Props) {
  const c = cfg.colors;
    ...
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: 280 }}>
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-14" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.7), transparent 65%)" }}>
          <h1 className="italic" style={{ fontSize: "clamp(28px,4vw,44px)", fontWeight: 400 }}>{collection.nom}</h1>
      ...
      <section style={{ padding: "50px 4vw 80px" }}>
        {produits.length > 0 ? <OrfevreProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-16 text-center" style={{ color: c.texteMuted }}>Aucun produit dans cette collection pour le moment.</p>}
      </section>
