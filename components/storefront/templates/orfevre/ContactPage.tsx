...
export function OrfevreContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "50px 4vw 10px" }}>
        <h1 className="text-[34px] italic" style={{ fontWeight: 400 }}>Contact</h1>
        <p className="mt-2 text-sm uppercase tracking-wide" style={{ color: c.texteMuted }}><Link href={`/${slug}`}>{tenant.nomBoutique}</Link> · Contact</p>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          <div className="space-y-3">
            ...
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ backgroundColor: c.surface, borderRadius: cfg.radius }}>
