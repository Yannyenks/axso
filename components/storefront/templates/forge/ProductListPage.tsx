...
export function ForgeProductListPage({ tenant, cfg, produits, slug, filtres }: Props) {
  const c = cfg.colors;
    ...
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section style={{ padding: "60px 4vw 40px" }}>
        <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-6 mb-9">
          ...
          <form method="GET" className="flex items-center gap-3 flex-wrap text-sm font-mono">
            ...
            <button type="submit" className="px-4 py-2 font-bold uppercase" style={{ backgroundColor: c.accent, color: c.fond }}>Filtrer</button>
          </form>
        </div>
        {collections.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-9 text-[11.5px] uppercase">
        ...
        {produits.length > 0 ? <ForgeProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-20 text-center font-mono" style={{ color: c.texteMuted }}>Aucun produit ne correspond à votre recherche.</p>}
      </section>
