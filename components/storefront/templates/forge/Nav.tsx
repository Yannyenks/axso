...
export function ForgeNav({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between backdrop-blur-md" style={{ padding: "18px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}>
      <Link href={`/${slug}`} className="text-2xl uppercase font-extrabold">
        {tenant.logoUrl ? <img src={tenant.logoUrl} alt={tenant.nomBoutique} className="h-8 object-contain" /> : tenant.nomBoutique}
      </Link>
      <nav className="hidden md:flex">
        <ul className="flex gap-7 list-none text-[12.5px] uppercase tracking-wide">{navLinks.map((l) => <li key={l.href}><Link href={l.href}>{l.label}</Link></li>)}</ul>
      </nav>
      <div className="flex items-center gap-4 text-[11.5px] uppercase">
        <div className="hidden sm:block relative" ref={searchBoxRef}>
          <input value={query} onChange={(e) => onQueryChange(e.target.value)} onFocus={() => resultats.length && setDropdownOuvert(true)}
            placeholder="Recherche" className="w-24 focus:w-40 transition-all bg-transparent outline-none border-b normal-case" style={{ borderColor: c.texteMuted, color: c.texte }} />
            ...
            <div className="absolute top-full right-0 mt-3 w-72 z-50" style={{ backgroundColor: c.surface, border: `1px solid ${c.bordure}` }}>
              {resultats.map((p) => (
                <Link key={p.id} href={`/${slug}/produits/${p.id}`} onClick={() => setDropdownOuvert(false)} className="flex items-center gap-3 px-3 py-2.5 normal-case" style={{ borderBottom: `1px solid ${c.bordure}` }}>
                  ...
                  <div className="min-w-0 flex-1"><p className="text-[12px] truncate">{p.nom}</p><p className="text-[11px]" style={{ color: c.texteMuted }}>{formatMontant(p.prix, devise)}</p></div>
                </Link>
              ))}
            </div>
          )}
        ...
        <button className="md:hidden normal-case" onClick={() => setMenuOuvert((v) => !v)} aria-label="Menu">☰</button>
      </div>
      {menuOuvert && (
        <div className="md:hidden absolute top-full left-0 right-0 z-40 flex flex-col gap-1 py-4 px-6" style={{ backgroundColor: c.fond, borderBottom: `1px solid ${c.bordure}` }}>
          {navLinks.map((l) => <Link key={l.href} href={l.href} onClick={() => setMenuOuvert(false)} className="py-2.5 text-[13px] uppercase">{l.label}</Link>)}
        </div>
      )}
    </header>
  );
