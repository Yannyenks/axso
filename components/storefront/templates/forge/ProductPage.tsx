...
export function ForgeProductPage({ tenant, cfg, slug, produit, prixAffiche, prixCompareAffiche, produitsSimilaires }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      <header className="sticky top-0 z-40 flex items-center justify-between backdrop-blur-md" style={{ padding: "16px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}>
        <Link href={`/${slug}`} className="text-lg uppercase font-extrabold">{tenant.nomBoutique}</Link>
      ...
      <section className="grid lg:grid-cols-[90px_1fr_1fr] gap-6" style={{ padding: "50px 4vw" }}>
        ...
        <div className="relative overflow-hidden order-1 lg:order-2" style={{ aspectRatio: "1/1", backgroundColor: c.surface }}>
          {images[actif] ? <img src={images[actif]} alt={produit.nom} className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ background: `radial-gradient(120% 90% at 40% 20%, ${c.accentSecondaire || c.accent}66, ${c.surface} 70%)` }} />}
        </div>
        <div className="order-3 flex flex-col justify-center">
          {produit.categorie && <div className="mb-3 text-xs" style={{ color: c.texteMuted }}>{produit.categorie.toUpperCase()}</div>}
          <h1 className="text-[30px] mb-4">{produit.nom}</h1>
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-lg font-bold" style={{ color: c.accent }}>{formatMontant(prixAffiche, devise)}</span>
          ...
          {produit.description && <p className="mb-7 leading-relaxed font-mono text-[13px]" style={{ color: c.texteMuted, maxWidth: "42ch" }}>{produit.description}</p>}
          {Object.entries(dimensions).map(([dim, variantes]) => (
            <div key={dim} className="mb-6">
              <p className="mb-2.5 text-xs" style={{ color: c.texteMuted }}>{dim.toUpperCase()}</p>
              <div className="flex flex-wrap gap-2">
                {variantes.map((v: any) => (
                  <button key={v.id} onClick={() => setSelection((s) => ({ ...s, [dim]: v.id }))} disabled={v.stock === 0}
                    className="px-4 py-2 text-sm font-bold disabled:opacity-30" style={{ border: `1.5px solid ${c.bordure}`, backgroundColor: selection[dim] === v.id ? c.accent : "transparent" }}>{v.valeur}</button>
                ))}
              </div>
            </div>
          ))}
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl">
            {produit.avis.slice(0, 6).map((a: any) => (
              <div key={a.id}>
                ...
                {a.commentaire && <p className="text-[13px] font-mono mb-2" style={{ color: c.texteMuted }}>{a.commentaire}</p>}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]" style={{ backgroundColor: c.bordure }}>
            {produitsSimilaires.map((p) => (
              <Link key={p.id} href={`/${slug}/produits/${p.id}`} style={{ backgroundColor: c.fond }}>
                ...
                <div className="p-3"><p className="text-sm font-bold uppercase truncate">{p.nom}</p><p className="text-xs" style={{ color: c.texteMuted }}>{formatMontant(p.prixAffiche, devise)}</p></div>
              </Link>
            ))}
          </div>
        </section>
