...
export function ForgeCollectionPage({ tenant, cfg, collection, produits, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: 280 }}>
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-14" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.65), transparent 65%)" }}>
          <h1 className="text-white" style={{ fontSize: "clamp(28px,4vw,44px)" }}>{collection.nom}</h1>
          {collection.description && <p className="mt-2 max-w-xl text-white/85 font-mono text-sm">{collection.description}</p>}
        </div>
      </section>
      <section style={{ padding: "50px 4vw 80px" }}>
        {produits.length > 0 ? <ForgeProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-16 text-center font-mono" style={{ color: c.texteMuted }}>Aucun produit dans cette collection pour le moment.</p>}
      </section>
