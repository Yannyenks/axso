...
export function AtlasHomePage({ tenant, cfg, vedettes, slug }: Props) {
  const c = cfg.colors;
  ...
  const NODES: Record<string, React.ReactNode> = {
    ...
    hero: sec.hero.actif ? (
      <section className="grid lg:grid-cols-[0.95fr_1.05fr] border-b" style={{ minHeight: "88vh", borderColor: c.bordure }}>
        <div className="flex flex-col justify-center border-r" style={{ padding: "5vw 3vw 5vw 4vw", borderColor: c.bordure }}>
          ...
          <h1 className="max-w-[11ch]" style={{ fontSize: "clamp(40px,5vw,64px)", lineHeight: 1.02 }}>{sec.hero.titre}</h1>
        ...
        <div className="relative overflow-hidden flex items-center justify-center" style={{ padding: "4vw", background: tenant.bannerUrl ? undefined : `radial-gradient(120% 100% at 60% 20%, ${c.accent} 0%, #a3421a 60%, #1B1B1E 100%)` }}>
    ...
    confiance: sec.confiance?.actif && (sec.confiance.items?.length ?? 0) > 0 ? (
      <section className="py-3 overflow-hidden" style={{ backgroundColor: c.texte }}>
        <div className="ax-atlas-marquee-track flex items-center w-max font-mono text-xs" style={{ color: c.fond }}>
        ...
        <style>{`.ax-atlas-marquee-track{animation:axAtlasMarquee 24s linear infinite;} .ax-atlas-marquee-track:hover{animation-play-state:paused;} @keyframes axAtlasMarquee{from{transform:translateX(0);}to{transform:translateX(-50%);}} @media(prefers-reduced-motion:reduce){.ax-atlas-marquee-track{animation:none;}}`}</style>
      </section>
    ) : null,
    ...
    vedettes: sec.vedettes.actif && vedettes.length > 0 ? (
      <section style={{ padding: "90px 4vw" }}>
        ...
        <AtlasProductGrid produits={vedettes} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} />
      </section>
    ) : null,
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {collections.slice(0, 4).map((col) => (
            <Link key={col.slug} href={`/${slug}/collections/${col.slug}`} className="relative overflow-hidden flex items-end p-5" style={{ aspectRatio: "3/4", backgroundColor: c.surface }}>
              {col.imageUrl ? <img src={col.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0" style={{ background: `linear-gradient(160deg, ${c.accent}, #6c6c68)` }} />}
    ...
    about: sec.about?.actif ? (
      <section style={{ padding: "70px 4vw" }}>
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="relative overflow-hidden" style={{ aspectRatio: "4/3", background: sec.about.imageUrl ? undefined : `radial-gradient(120% 90% at 40% 20%, ${c.accent}44, ${c.surface} 70%)` }}>{sec.about.imageUrl && <img src={sec.about.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover" />}</div>
          <div>
            {sec.about.badgeTexte && <div className="mb-3 text-xs font-mono" style={{ color: c.texteMuted }}>{sec.about.badgeTexte.toUpperCase()}</div>}
