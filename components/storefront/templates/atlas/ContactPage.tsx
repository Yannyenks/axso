...
export function AtlasContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
            ...
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ backgroundColor: c.surface }}>
                  ...
                  <div className="min-w-0"><p className="text-xs font-mono" style={{ color: c.texteMuted }}>{item.label.toUpperCase()}</p><p className="text-sm font-medium truncate">{item.value}</p></div>
                </div>
              );
