...
export function PulseContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "50px 4vw 10px" }}>
        <h1 className="text-[36px] font-bold uppercase">Contact</h1>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          <div className="space-y-3">
            ...
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ border: `2px solid ${c.bordure}` }}>
                  <item.Icon size={16} style={{ color: c.accent }} />
                  <div className="min-w-0">
                    <p className="text-xs uppercase tracking-wide font-bold" style={{ color: c.texteMuted }}>{item.label}</p>
