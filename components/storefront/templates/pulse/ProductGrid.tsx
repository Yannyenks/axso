...
function fallbackGradient(accent: string, secondaire: string, i: number): string {
  const angles = [
    `linear-gradient(160deg, ${accent}, ${secondaire})`,
    `linear-gradient(160deg, ${secondaire}, #0E0E12)`,
    `linear-gradient(160deg, #63636B, #0E0E12)`,
  ];
...
export function PulseProductGrid({ produits, slug, devise, commissionRate, colors: c, colonnes = 3 }: Props) {
  if (!produits.length) return null;
  return (
    <div className={`grid grid-cols-2 ${colonnes === 4 ? "sm:grid-cols-4" : "sm:grid-cols-3"} gap-[2px]`} style={{ backgroundColor: c.bordure }}>
      {produits.map((p, i) => {
        ...
        return (
          <Link key={p.id} href={`/${slug}/produits/${p.id}`} className="relative overflow-hidden group" style={{ backgroundColor: c.fond }}>
            <div className="relative overflow-hidden" style={{ aspectRatio: "1/1", background: img1 ? undefined : fallbackGradient(c.accent, c.accentSecondaire || c.accent, i) }}>
              {p.featured && (
                <span className="absolute top-3.5 left-3.5 z-10 text-[11px] font-bold px-2.5 py-1" style={{ backgroundColor: c.accent, color: "#fff" }}>
                  NOUVEAU
                </span>
              )}
            ...
            <div className="flex items-baseline justify-between px-4.5 py-4">
              ...
              <span className="text-[13px] font-bold flex-shrink-0" style={{ color: c.texteMuted }}>{formatMontant(prixAffiche, devise)}</span>
            </div>
          </Link>
        );
      })}
