...
export function PulseNav({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <header
      ...
      style={{ padding: "20px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}
    >
      <Link href={`/${slug}`} className="text-2xl font-bold tracking-tight" style={{ letterSpacing: "-0.02em" }}>
        {tenant.logoUrl ? <img src={tenant.logoUrl} alt={tenant.nomBoutique} className="h-8 object-contain" /> : tenant.nomBoutique}
      </Link>

      <nav className="hidden md:flex">
        <ul className="flex gap-8 list-none text-sm font-semibold">
          {navLinks.map((l) => (
            <li key={l.href}>
              <Link href={l.href} className="axs-pulse-navlink relative pb-0.5">{l.label}</Link>
            </li>
          ))}
        </ul>
      </nav>
      ...
      <div className="flex items-center gap-4 text-sm font-semibold">
        <div className="hidden sm:block relative" ref={searchBoxRef}>
          <input
            ...
            className="w-28 focus:w-44 transition-all bg-transparent outline-none border-b font-normal"
            style={{ borderColor: c.texteMuted, color: c.texte }}
          />
              ...
              {resultats.map((p) => (
                <Link key={p.id} href={`/${slug}/produits/${p.id}`} onClick={() => setDropdownOuvert(false)}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[12.5px] truncate font-semibold">{p.nom}</p>
                    <p className="text-[11.5px] font-normal" style={{ color: c.texteMuted }}>{formatMontant(p.prix, devise)}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <Link href={`/${slug}/panier`} className="px-4 py-2" style={{ backgroundColor: c.texte, color: c.fond }}>
          Panier ({totalItems})
        </Link>
      ...
      {menuOuvert && (
        <div className="md:hidden absolute top-full left-0 right-0 z-40 flex flex-col gap-1 py-4 px-6" style={{ backgroundColor: c.fond, borderBottom: `1px solid ${c.bordure}` }}>
          {navLinks.map((l) => (
            <Link key={l.href} href={l.href} onClick={() => setMenuOuvert(false)} className="py-2.5 text-sm font-semibold">{l.label}</Link>
          ))}
        </div>
      )}

      <style>{`.axs-pulse-navlink::after{content:'';position:absolute;left:0;bottom:-2px;width:0;height:2px;background:${c.accent};transition:width .3s ease;} .axs-pulse-navlink:hover::after{width:100%;}`}</style>
    </header>
  );
}
