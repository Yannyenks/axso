...
export function HaloAboutPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "50px 4vw 10px" }}>
        <h1 className="text-[34px]">À propos</h1>
        <p className="mt-2 text-sm" style={{ color: c.texteMuted }}><Link href={`/${slug}`}>{tenant.nomBoutique}</Link> · À propos</p>
      </div>
      {aboutPage?.actif && aboutPage.sections?.length ? (
        ...
        <section style={{ padding: "30px 4vw 80px" }}>
          <div className="max-w-2xl">
            ...
            <p className="leading-relaxed" style={{ color: c.texteMuted }}>{about?.texte || tenant.description || "Cette boutique n'a pas encore renseigné sa page À propos."}</p>
          </div>
        </section>
      )}
