...
export function HaloContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "50px 4vw 10px" }}>
        <h1 className="text-[34px]">Contact</h1>
            ...
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ backgroundColor: c.surface }}>
                  ...
                  <div className="min-w-0"><p className="text-xs uppercase tracking-wide" style={{ color: c.texteMuted }}>{item.label}</p><p className="text-sm truncate">{item.value}</p></div>
                </div>
              );
