...
export function HaloProductListPage({ tenant, cfg, produits, slug, filtres }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section style={{ padding: "60px 4vw 40px" }}>
        ...
        {produits.length > 0 ? <HaloProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-20 text-center" style={{ color: c.texteMuted }}>Aucun produit ne correspond à votre recherche.</p>}
      </section>
