...
export function HaloProductPage({ tenant, cfg, slug, produit, prixAffiche, prixCompareAffiche, produitsSimilaires }: Props) {
  const c = cfg.colors;
    ...
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section className="grid lg:grid-cols-[90px_1fr_1fr] gap-6" style={{ padding: "50px 4vw" }}>
        <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-visible order-2 lg:order-1">
          {images.map((img, i) => (
            <button key={i} onClick={() => setActif(i)} className="flex-shrink-0 overflow-hidden" style={{ width: 90, height: 64, border: `1px solid ${i === actif ? c.accent : "transparent"}` }}>
              <img src={img} alt="" className="w-full h-full object-cover" />
            </button>
          ))}
        </div>
        <div className="relative overflow-hidden order-1 lg:order-2" style={{ aspectRatio: "1/0.82", backgroundColor: c.surface }}>
        ...
        <div className="order-3 flex flex-col justify-center">
          <div className="mb-3 text-[12.5px] font-semibold" style={{ color: c.accent }}>{produit.nom.toUpperCase()}</div>
          <h1 className="text-[30px] mb-4">{produit.nom}</h1>
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-[19px] font-semibold" style={{ color: c.accent }}>{formatMontant(prixAffiche, devise)}</span>
          ...
          {produit.description && <p className="mb-7 leading-relaxed" style={{ color: c.texteMuted, maxWidth: "42ch" }}>{produit.description}</p>}
              ...
              <div className="flex flex-wrap gap-2.5">
                {variantes.map((v: any) => (
                  <button key={v.id} onClick={() => setSelection((s) => ({ ...s, [dim]: v.id }))} disabled={v.stock === 0}
                    className="w-7 h-7 rounded-full disabled:opacity-30" style={{ border: `2px solid ${selection[dim] === v.id ? c.texte : "transparent"}`, backgroundColor: v.image ? undefined : c.accent }} title={v.valeur} />
                ))}
              </div>
            </div>
          ))}
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl">
            {produit.avis.slice(0, 6).map((a: any) => (
              <div key={a.id}>
                ...
                {a.titre && <p className="mb-1.5">{a.titre}</p>}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]" style={{ backgroundColor: c.bordure }}>
            {produitsSimilaires.map((p) => (
              <Link key={p.id} href={`/${slug}/produits/${p.id}`} style={{ backgroundColor: c.fond }}>
                <div className="relative overflow-hidden" style={{ aspectRatio: "1/0.8" }}>{p.images[0] ? <img src={p.images[0]} alt={p.nom} className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ backgroundColor: c.surface }} />}</div>
