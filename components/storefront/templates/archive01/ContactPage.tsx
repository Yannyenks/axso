...
export function Archive01ContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "50px 4vw 10px" }}>
        <h1 className="text-[34px] uppercase">Contact</h1>
        <p className="mt-2 text-sm font-bold uppercase" style={{ color: c.texteMuted }}><Link href={`/${slug}`}>{tenant.nomBoutique}</Link> · Contact</p>
        {contactPage?.intro && <p className="mt-4 max-w-xl font-medium" style={{ color: c.texteMuted }}>{contactPage.intro}</p>}
      </div>
      <section style={{ padding: "30px 4vw 80px" }}>
        <div className="grid lg:grid-cols-2 gap-10">
          <div className="space-y-3">
            {coordonnees.length === 0 && <p className="text-sm font-medium" style={{ color: c.texteMuted }}>Aucune coordonnée renseignée pour l'instant.</p>}
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ border: `1px solid ${c.bordure}` }}>
                  ...
                  <div className="min-w-0"><p className="text-[11px] font-bold uppercase" style={{ color: c.texteMuted }}>{item.label}</p><p className="text-sm font-semibold truncate">{item.value}</p></div>
                </div>
              );
