...
export function Archive01ProductPage({ tenant, cfg, slug, produit, prixAffiche, prixCompareAffiche, produitsSimilaires }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      <header className="sticky top-0 z-40 flex items-center justify-between backdrop-blur-md" style={{ padding: "16px 4vw", backgroundColor: `${c.fond}eb`, borderBottom: `1px solid ${c.bordure}` }}>
        <Link href={`/${slug}`} className="text-lg uppercase">{tenant.nomBoutique}</Link>
        <Link href={`/${slug}/produits`} className="text-[13px] font-bold uppercase">← Tous les produits</Link>
      </header>
      ...
      <section className="grid lg:grid-cols-[90px_1fr_1fr] gap-6" style={{ padding: "50px 4vw" }}>
        ...
        <div className="order-3 flex flex-col justify-center">
          {produit.categorie && <div className="mb-3 text-[11.5px] font-bold uppercase" style={{ color: c.texteMuted }}>{produit.categorie}</div>}
          <h1 className="text-[30px] uppercase mb-4">{produit.nom}</h1>
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-[20px]" style={{ color: c.accent }}>{formatMontant(prixAffiche, devise)}</span>
            {prixCompareAffiche && prixCompareAffiche > prixAffiche && <span className="line-through font-semibold" style={{ color: c.texteMuted }}>{formatMontant(prixCompareAffiche, devise)}</span>}
          </div>
          {noteMoyenne > 0 && (
          ...
          {Object.entries(dimensions).map(([dim, variantes]) => (
            <div key={dim} className="mb-6">
              <p className="mb-2.5 text-[11.5px] font-bold uppercase" style={{ color: c.texteMuted }}>{dim}</p>
              <div className="grid grid-cols-6 gap-2 max-w-[340px]">
                {variantes.map((v: any) => (
                  <button key={v.id} onClick={() => setSelection((s) => ({ ...s, [dim]: v.id }))} disabled={v.stock === 0}
                    className="py-2.5 text-[13px] font-bold disabled:opacity-30" style={{ border: `1.5px solid ${c.bordure}`, backgroundColor: selection[dim] === v.id ? c.accent : "transparent" }}>{v.valeur}</button>
                ))}
              </div>
            </div>
          ))}
      ...
      {(produit.avis?.length ?? 0) > 0 && (
        <section style={{ padding: "20px 4vw 70px", borderTop: `1px solid ${c.bordure}` }}>
          <h2 className="text-[24px] uppercase mb-8 mt-10">Avis clients</h2>
      ...
      {produitsSimilaires.length > 0 && (
        <section style={{ padding: "20px 4vw 80px", borderTop: `1px solid ${c.bordure}` }}>
          <h2 className="text-[24px] uppercase mb-8 mt-10">Vous aimerez aussi</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]" style={{ backgroundColor: c.bordure }}>
            {produitsSimilaires.map((p) => (
              <Link key={p.id} href={`/${slug}/produits/${p.id}`} style={{ backgroundColor: c.fond }}>
                ...
                <div className="p-3"><p className="text-[13px] font-bold uppercase truncate">{p.nom}</p><p className="text-[12px] font-medium" style={{ color: c.texteMuted }}>{formatMontant(p.prixAffiche, devise)}</p></div>
              </Link>
            ))}
          </div>
        </section>
