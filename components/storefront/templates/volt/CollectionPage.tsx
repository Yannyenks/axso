...
export function VoltCollectionPage({ tenant, cfg, collection, produits, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: 280 }}>
        {collection.imageUrl ? <img src={collection.imageUrl} alt={collection.nom} className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0" style={{ background: `radial-gradient(120% 100% at 60% 20%, ${c.accent} 0%, #0c211e 100%)` }} />}
      ...
      <section style={{ padding: "50px 4vw 80px" }}>
        {produits.length > 0 ? <VoltProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-16 text-center font-medium" style={{ color: c.texteMuted }}>Aucun produit dans cette collection pour le moment.</p>}
      </section>
