...
import { VoltProductGrid } from "./ProductGrid";

interface Props { tenant: any; cfg: ThemeConfig; vedettes: any[]; slug: string }
const DEFAULT_ORDER = ["annonce", "hero", "confiance", "about", "vedettes", "collections", "promo", "faq", "avis", "newsletter"];

export function VoltHomePage({ tenant, cfg, vedettes, slug }: Props) {
  const c = cfg.colors;
  ...
  const NODES: Record<string, React.ReactNode> = {
    annonce: sec.annonce.actif ? (<div className="py-2.5 text-center text-xs font-bold" style={{ backgroundColor: sec.annonce.couleurFond, color: sec.annonce.couleurTexte }}>{sec.annonce.texte}</div>) : null,

    hero: sec.hero.actif ? (
      <section className="grid lg:grid-cols-[0.9fr_1.1fr] border-b" style={{ minHeight: "88vh", borderColor: c.bordure }}>
        <div className="flex flex-col justify-center border-r" style={{ padding: "5vw 3vw 5vw 4vw", borderColor: c.bordure }}>
          {sec.hero.badgeTexte && <div className="mb-5 text-[11.5px] font-mono" style={{ color: c.texteMuted }}>{sec.hero.badgeTexte.toUpperCase()}</div>}
          <h1 className="max-w-[10ch]" style={{ fontSize: "clamp(42px,5.4vw,70px)", lineHeight: 1.02 }}>{sec.hero.titre}</h1>
          <p className="mt-5.5 max-w-[36ch] font-medium" style={{ color: c.texteMuted }}>{sec.hero.sousTitre}</p>
          <Link href={ctaHref} className="inline-flex mt-8 px-7 py-4 font-bold text-sm transition-colors" style={{ backgroundColor: c.accent, color: c.fond }}>{sec.hero.ctaTexte}</Link>
        </div>
        <div className="relative overflow-hidden flex items-center justify-center" style={{ padding: "4vw", background: tenant.bannerUrl ? undefined : `radial-gradient(120% 100% at 60% 20%, ${c.accent} 0%, #067366 60%, #0c211e 100%)` }}>
    ...
    confiance: sec.confiance?.actif && (sec.confiance.items?.length ?? 0) > 0 ? (
      <section className="py-3 overflow-hidden" style={{ backgroundColor: c.accent }}>
        <div className="ax-volt-marquee-track flex items-center w-max font-mono text-xs" style={{ color: c.fond }}>
        ...
        <style>{`.ax-volt-marquee-track{animation:axVoltMarquee 24s linear infinite;} .ax-volt-marquee-track:hover{animation-play-state:paused;} @keyframes axVoltMarquee{from{transform:translateX(0);}to{transform:translateX(-50%);}} @media(prefers-reduced-motion:reduce){.ax-volt-marquee-track{animation:none;}}`}</style>
      </section>
    ) : null,

    about: sec.about?.actif ? (
      <section style={{ padding: "90px 4vw", borderBottom: `1px solid ${c.bordure}` }}>
        <div className="flex justify-between items-end mb-9">
          <h2 className="text-[32px]">{sec.about.titre}</h2>
          {sec.about.badgeTexte && <span className="text-xs font-mono" style={{ color: c.texteMuted }}>{sec.about.badgeTexte.toUpperCase()}</span>}
        </div>
        {sec.about.stats?.length ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]" style={{ backgroundColor: c.bordure }}>
            {sec.about.stats.map((s, i) => (
              <div key={i} className="p-6" style={{ backgroundColor: c.fond }}>
                <div className="text-xs font-mono mb-2.5" style={{ color: c.texteMuted }}>{s.label.toUpperCase()}</div>
                <b className="text-[21px] font-extrabold block">{s.valeur}</b>
              </div>
            ))}
          </div>
        ) : (
    ...
    vedettes: sec.vedettes.actif && vedettes.length > 0 ? (
      <section style={{ padding: "90px 4vw" }}>
        <div className="flex justify-between items-end mb-9">
          ...
          <span className="hidden sm:block text-xs font-mono" style={{ color: c.texteMuted }}>SURVOLEZ POUR VOIR LE PROFIL</span>
        </div>
        <VoltProductGrid produits={vedettes} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} />
      </section>
    ) : null,
    ...
    promo: sec.promo.actif ? (
      <section style={{ padding: "20px 4vw 70px" }}>
        <div className="text-center py-16 px-8" style={{ backgroundColor: c.surface }}>
          ...
          <Link href={`/${slug}/produits`} className="inline-flex px-7 py-3.5 font-bold text-sm" style={{ backgroundColor: c.accent, color: c.fond }}>{sec.promo.ctaTexte}</Link>
        </div>
      </section>
    ) : null,
    ...
    faq: sec.faq?.actif && (sec.faq.items?.length ?? 0) > 0 ? (
      <section style={{ padding: "20px 4vw 70px" }}>
        ...
        <div className="max-w-2xl space-y-6">{sec.faq.items.map((it, i) => <div key={i}><p className="font-bold mb-1.5">{it.question}</p><p className="text-[13.5px] font-medium" style={{ color: c.texteMuted }}>{it.reponse}</p></div>)}</div>
      </section>
    ) : null,
      ...
      <section style={{ padding: "20px 4vw 90px" }}>
        <div className="max-w-xl mx-auto text-center">
          ...
          <form method="post" action="/api/newsletter" className="flex flex-col sm:flex-row gap-2 justify-center">
            <input type="email" name="email" placeholder={sec.newsletter.placeholder} className="flex-1 max-w-sm px-4 py-3 text-sm outline-none border" style={{ borderColor: c.bordure, backgroundColor: c.surface }} />
            <button type="submit" className="px-6 py-3 text-sm font-bold" style={{ backgroundColor: c.accent, color: c.fond }}>{sec.newsletter.ctaTexte}</button>
          </form>
        </div>
      </section>
    ) : null,
