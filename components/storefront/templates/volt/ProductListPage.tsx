...
export function VoltProductListPage({ tenant, cfg, produits, slug, filtres }: Props) {
  const c = cfg.colors;
    ...
    <div style={{ backgroundColor: c.fond, color: c.texte, minHeight: "100vh" }}>
      ...
      <section style={{ padding: "60px 4vw 40px" }}>
        <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-6 mb-9">
          <h1 className="text-[30px]">Tous les produits</h1>
          <form method="GET" className="flex items-center gap-3 flex-wrap text-sm font-medium">
            ...
            <button type="submit" className="px-4 py-2 font-bold" style={{ backgroundColor: c.accent, color: c.fond }}>Filtrer</button>
          </form>
        </div>
        {collections.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-9 text-[12px] font-bold">
            ...
            {collections.map((col) => <Link key={col.slug} href={`/${slug}/produits?collection=${col.slug}`} className="px-3.5 py-2 border" style={{ borderColor: c.bordure, color: filtres.collection === col.slug ? c.fond : c.texte, backgroundColor: filtres.collection === col.slug ? c.texte : "transparent" }}>{col.nom}</Link>)}
          </div>
        )}
        {produits.length > 0 ? <VoltProductGrid produits={produits} slug={slug} devise={devise} commissionRate={commissionRate} colors={c} /> : <p className="py-20 text-center font-medium" style={{ color: c.texteMuted }}>Aucun produit ne correspond à votre recherche.</p>}
      </section>
