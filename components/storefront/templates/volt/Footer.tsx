...
export function VoltFooter({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  const navLinks = [
    ...
    <footer style={{ padding: "60px 4vw 30px" }}>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 pb-11" style={{ borderBottom: `1px solid ${c.bordure}` }}>
        <div className="lg:col-span-2">
          ...
          <p className="max-w-[30ch] text-[13.5px] font-medium" style={{ color: c.texteMuted }}>{tenant.description || "Mobilité électrique conçue pour durer."}</p>
        </div>
        <div>
          <p className="text-xs font-bold mb-4" style={{ color: c.texteMuted }}>PRODUITS</p>
        ...
        <div>
          <p className="text-xs font-bold mb-4" style={{ color: c.texteMuted }}>SUPPORT</p>
