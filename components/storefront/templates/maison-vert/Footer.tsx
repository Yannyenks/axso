...
export function MaisonVertFooter({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  const navLinks = [
  ...
  return (
    <footer style={{ padding: "60px 4vw 30px", textAlign: "center" }}>
      <ul className="flex flex-wrap justify-center gap-8 list-none text-sm font-medium mb-9">
        {navLinks.map((l) => <li key={l.href}><Link href={l.href}>{l.label}</Link></li>)}
      </ul>
      <div className="text-xs pt-6" style={{ color: c.texteMuted, borderTop: `1px solid ${c.bordure}` }}>
