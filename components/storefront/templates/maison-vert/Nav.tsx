...
export function MaisonVertNav({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between backdrop-blur-md" style={{ padding: "22px 4vw", backgroundColor: `${c.fond}e6`, borderBottom: `1px solid ${c.bordure}` }}>
      <Link href={`/${slug}`} className="text-[22px]">
        {tenant.logoUrl ? <img src={tenant.logoUrl} alt={tenant.nomBoutique} className="h-8 object-contain" /> : tenant.nomBoutique}
      </Link>
      <nav className="hidden md:flex">
        <ul className="flex gap-8 list-none text-sm font-medium">{navLinks.map((l) => <li key={l.href}><Link href={l.href}>{l.label}</Link></li>)}</ul>
      </nav>
      <div className="flex items-center gap-4 text-[13px]">
        <div className="hidden sm:block relative" ref={searchBoxRef}>
          <input value={query} onChange={(e) => onQueryChange(e.target.value)} onFocus={() => resultats.length && setDropdownOuvert(true)}
            placeholder="Recherche" className="w-28 focus:w-44 transition-all bg-transparent outline-none border-b" style={{ borderColor: c.texteMuted, color: c.texte }} />
          {dropdownOuvert && resultats.length > 0 && (
            <div className="absolute top-full right-0 mt-3 w-72 z-50 overflow-hidden" style={{ backgroundColor: c.fond, border: `1px solid ${c.bordure}`, borderRadius: cfg.radius }}>
              {resultats.map((p) => (
                <Link key={p.id} href={`/${slug}/produits/${p.id}`} onClick={() => setDropdownOuvert(false)} className="flex items-center gap-3 px-3 py-2.5" style={{ borderBottom: `1px solid ${c.bordure}` }}>
                  <div className="w-10 h-10 flex-shrink-0 overflow-hidden rounded-xl" style={{ backgroundColor: c.surface }}>{p.image && <img src={p.image} alt="" className="w-full h-full object-cover" />}</div>
        ...
        <Link href={`/${slug}/panier`} className="px-4 py-2 rounded-full" style={{ backgroundColor: c.accent, color: c.fond }}>Panier ({totalItems})</Link>
      ...
      {menuOuvert && (
        <div className="md:hidden absolute top-full left-0 right-0 z-40 flex flex-col gap-1 py-4 px-6" style={{ backgroundColor: c.fond, borderBottom: `1px solid ${c.bordure}` }}>
          {navLinks.map((l) => <Link key={l.href} href={l.href} onClick={() => setMenuOuvert(false)} className="py-2.5 text-sm font-medium">{l.label}</Link>)}
        </div>
      )}
    </header>
  );
