...
export function MaisonVertContactPage({ tenant, cfg, slug }: Props) {
  const c = cfg.colors;
            ...
            {coordonnees.map((item, i) => {
              const content = (
                <div className="flex items-center gap-3 p-4" style={{ backgroundColor: c.surface, borderRadius: cfg.radius }}>
                  ...
                  <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wide" style={{ color: c.texteMuted }}>{item.label}</p><p className="text-sm truncate">{item.value}</p></div>
                </div>
              );
