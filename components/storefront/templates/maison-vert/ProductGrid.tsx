...
function fallbackGradient(accent: string, secondaire: string, i: number): string {
  const angles = [`radial-gradient(120% 100% at 50% 20%, ${secondaire} 0%, ${accent} 60%, #16261a 100%)`, `radial-gradient(120% 100% at 50% 20%, ${accent} 0%, #2F4A34 60%, #16261a 100%)`, `radial-gradient(120% 100% at 50% 20%, #d8d2b8 0%, #b9b190 60%, #8f886a 100%)`];
  return angles[i % angles.length];
}

interface Props { produits: any[]; slug: string; devise: string; commissionRate: number; colors: ThemeColors; radius: string }

export function MaisonVertProductGrid({ produits, slug, devise, commissionRate, colors: c, radius }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
      {produits.map((p, i) => {
        ...
        return (
          <Link key={p.id} href={`/${slug}/produits/${p.id}`} className="overflow-hidden" style={{ backgroundColor: c.surface, borderRadius: radius }}>
            <div className="relative overflow-hidden group" style={{ aspectRatio: "1/1.1", background: img1 ? undefined : fallbackGradient(c.accent, c.accentSecondaire || c.accent, i) }}>
            ...
            <div className="flex items-baseline justify-between px-5 py-4.5">
              <span className="text-[16px] truncate pr-2">{p.nom}</span>
