    ...
    <main className="bg-[#080808] text-white min-h-screen" style={{ fontFamily: "'Poppins','Century Gothic',system-ui,sans-serif" }}>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-px rounded-2xl overflow-hidden"
          ...
          {STATS.map((s, i) => (
            <div key={s.label} className="py-10 px-6 text-center"
              style={{ background: i % 2 === 0 ? "rgba(245,166,35,0.04)" : "rgba(255,255,255,0.02)" }}>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="rounded-2xl overflow-hidden border relative"
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#111" }}>
              <div className="flex gap-1.5">{["#ff5f57","#febc2e","#28c840"].map(c => <div key={c} className="w-3 h-3 rounded-full" style={{ background: c }} />)}</div>
              <span className="text-white/25 text-xs ml-2">dashboard — Catalogue</span>
            </div>
              ...
              {[
                ...
                <div key={p.nom} className="flex items-center gap-4 rounded-xl p-3"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-semibold truncate">{p.nom}</p>
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: `${p.accent}12`, color: p.accent }}>{p.type}</span>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-white font-bold text-sm">{p.prix} XOF</p>
                    <p className="text-white/35 text-xs">Stock : {p.stock}</p>
                  </div>
                </div>
              ))}
              <div className="flex items-center justify-between pt-1">
                <span className="text-white/25 text-xs">127 produits au total</span>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="rounded-2xl overflow-hidden border order-2 lg:order-1"
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#111" }}>
              <div className="flex gap-1.5">{["#ff5f57","#febc2e","#28c840"].map(c => <div key={c} className="w-3 h-3 rounded-full" style={{ background: c }} />)}</div>
              <span className="text-white/25 text-xs ml-2">Page de paiement — Checkout</span>
            </div>
            <div className="p-5">
              <p className="text-white/50 text-xs font-bold uppercase tracking-wider mb-4">Choisissez votre mode de paiement</p>
              <div className="grid grid-cols-2 gap-2 mb-5">
                {[
                  ...
                  <div key={p.nom}
                    ...
                    style={{
                      background: p.active ? `${p.color}12` : "rgba(255,255,255,0.03)",
                    ...
                    <span className="text-xs font-medium" style={{ color: p.active ? p.color : "rgba(255,255,255,0.5)" }}>{p.nom}</span>
              ...
              <div className="rounded-xl p-4" style={{ background: "rgba(0,185,241,0.08)", border: "1px solid rgba(0,185,241,0.2)" }}>
                <div className="flex justify-between mb-2">
                  <span className="text-white/50 text-sm">Total commande</span>
                  <span className="text-white font-bold">24 500 XOF</span>
                </div>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="rounded-2xl overflow-hidden border"
            style={{ background: "#0d0d0d", borderColor: "rgba(124,58,237,0.25)" }}>
            <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: "rgba(124,58,237,0.15)", background: "rgba(124,58,237,0.06)" }}>
              ...
              <div>
                <p className="text-white font-bold text-sm">AXIA</p>
                <p className="text-white/40 text-xs">Agent e-commerce · En ligne</p>
              </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="max-w-[85%] rounded-2xl rounded-tl-sm px-4 py-3 text-sm space-y-2"
                  style={{ background: "rgba(124,58,237,0.12)", border: "1px solid rgba(124,58,237,0.2)", color: "rgba(255,255,255,0.8)" }}>
                  ...
                  {[["Robe wax premium","47 ventes","1,15M XOF"],["Bijoux argent","32 ventes","384K XOF"],["Formation e-comm","18 ventes","270K XOF"]].map(([n,v,r]) => (
                    <div key={n} className="flex justify-between text-xs rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.05)" }}>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          {/* Tracking mockup */}
          <div className="rounded-2xl overflow-hidden border order-2 lg:order-1"
            style={{ background: "#0d0d0d", borderColor: "rgba(59,130,246,0.2)" }}>
            <div className="px-5 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#111" }}>
              <p className="text-white/50 text-xs font-bold uppercase tracking-wider">Suivi commande #AX-2847</p>
              <p className="text-white font-bold mt-0.5">En cours de livraison</p>
            </div>
            <div className="p-5">
              {/* Fausse carte */}
                  ...
                  <div key={e.label} className="flex items-center gap-3">
                    <div className="flex flex-col items-center">
                      <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                        style={{ background: e.done ? (e.active ? "linear-gradient(135deg,#F5A623,#d4880d)" : "rgba(59,130,246,0.8)") : "rgba(255,255,255,0.08)" }}>
                        {e.done && <Check size={10} color="#fff" strokeWidth={3} />}
                      </div>
                      {i < 3 && <div className="w-0.5 h-4" style={{ background: e.done ? "rgba(59,130,246,0.4)" : "rgba(255,255,255,0.06)" }} />}
                    </div>
                    <div className="flex-1 flex justify-between">
                      <span className="text-sm" style={{ color: e.active ? "#F5A623" : e.done ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.3)" }}>{e.label}</span>
                      <span className="text-xs" style={{ color: e.done ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.2)" }}>{e.heure}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          {/* Metrics mockup */}
          <div className="rounded-2xl overflow-hidden border"
            style={{ background: "#0d0d0d", borderColor: "rgba(239,68,68,0.2)" }}>
            <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#111" }}>
              <div className="flex gap-1.5">{["#ff5f57","#febc2e","#28c840"].map(c => <div key={c} className="w-3 h-3 rounded-full" style={{ background: c }} />)}</div>
              <span className="text-white/25 text-xs ml-2">Marketing — Performances</span>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={m.label} className="rounded-xl p-3 text-center" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <p className="text-white font-black text-lg">{m.val}</p>
              ...
              <div>
                <p className="text-white/35 text-xs font-bold uppercase tracking-wider mb-3">Automations actives</p>
                {[
                  ...
                  <div key={a.nom} className="flex items-center gap-3 rounded-xl px-3 py-2.5 mb-2"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-xs font-semibold truncate">{a.nom}</p>
                      <p className="text-white/35 text-[10px]">{a.envois}</p>
                    </div>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-32"
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-white mb-4">L'équipe IA qui ne dort jamais</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {AGENTS.map(({ icon: Icon, nom, desc }) => (
              <div key={nom}
                ...
                style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(124,58,237,0.12)" }}>
                ...
                <h3 className="font-bold text-white mb-2">{nom}</h3>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-24">
        <div className="max-w-3xl mx-auto rounded-3xl p-12 text-center border relative overflow-hidden"
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-white mb-4 relative">Prêt à bâtir votre empire ?</h2>
          <p className="text-white/50 text-lg mb-8 relative">Créez votre boutique en 2 minutes. Gratuit, sans carte bancaire.</p>
          <Link href="/inscription"
          ...
          <p className="text-white/25 text-xs mt-4 relative">
