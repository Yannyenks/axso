    ...
    <main className="bg-[#080808] text-white min-h-screen" style={{ fontFamily: "'Poppins','Century Gothic',system-ui,sans-serif" }}>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-20">
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-5 mb-24">
          {CHANNELS.map(({ Icon, titre, desc, lien, label, accent }) => (
            <a key={titre} href={lien} target={lien.startsWith("http") ? "_blank" : undefined}
              ...
              <h3 className="font-bold text-white mb-1.5">{titre}</h3>
              <p className="text-white/40 text-sm mb-5 leading-relaxed">{desc}</p>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-2xl p-6 border" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-bold text-white text-sm">Horaires d'assistance</h3>
              </div>
              <div className="space-y-2 text-sm text-white/50">
                <div className="flex justify-between"><span>Lun – Ven</span><span className="text-white/70">08h00 – 20h00</span></div>
                <div className="flex justify-between"><span>Samedi</span><span className="text-white/70">09h00 – 17h00</span></div>
                <div className="flex justify-between"><span>Dimanche</span><span className="text-white/35">Fermé</span></div>
              </div>
            </div>
            ...
            <div className="rounded-2xl p-6 border" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
              ...
              <p className="text-white/50 text-sm leading-relaxed">Dakar, Sénégal<br />Équipe distribuée sur 12 pays</p>
            </div>

            <div className="rounded-2xl p-6 border" style={{ background: "rgba(37,211,102,0.04)", borderColor: "rgba(37,211,102,0.15)" }}>
              <h3 className="font-bold text-white text-sm mb-2">Partenariats & Presse</h3>
              <p className="text-white/45 text-xs leading-relaxed mb-3">Pour les demandes médias, partenariats ou investisseurs :</p>
          ...
          <div className="lg:col-span-3">
            <div className="rounded-3xl p-8 border"
              style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(245,166,35,0.15)" }}>
              <h2 className="text-2xl font-black mb-1 text-white">Envoyer un message</h2>
              <p className="text-white/40 text-sm mb-7">Nous répondons sous 24h en jours ouvrés.</p>

              <style>{`
                .axso-input { background:#0d0d0d; border:1px solid rgba(255,255,255,0.1); color:#fff; width:100%; border-radius:12px; padding:14px 16px; font-size:14px; outline:none; transition:all .2s; font-family:inherit; }
              ...
              <form className="space-y-4" action="mailto:support@axso.app" method="get">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-white/55 text-xs font-semibold mb-1.5 uppercase tracking-wide">Prénom</label>
                    <input type="text" name="fname" placeholder="Aminata" className="axso-input" />
                  </div>
                  <div>
                    <label className="block text-white/55 text-xs font-semibold mb-1.5 uppercase tracking-wide">Nom</label>
                    <input type="text" name="lname" placeholder="Diallo" className="axso-input" />
                  </div>
                </div>
                <div>
                  <label className="block text-white/55 text-xs font-semibold mb-1.5 uppercase tracking-wide">Email</label>
                  <input type="email" name="email" placeholder="aminata@example.com" className="axso-input" />
                </div>
                <div>
                  <label className="block text-white/55 text-xs font-semibold mb-1.5 uppercase tracking-wide">Sujet</label>
                  <input type="text" name="subject" placeholder="Support technique, partenariat, presse..." className="axso-input" />
                </div>
                <div>
                  <label className="block text-white/55 text-xs font-semibold mb-1.5 uppercase tracking-wide">Message</label>
      ...
      <section className="px-6 sm:px-10 lg:px-16 xl:px-24 pb-28">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-black text-white text-center mb-10">Questions fréquentes</h2>
          <div className="space-y-3">
            {FAQ.map((item, i) => (
              <div key={i} className="rounded-2xl p-6 border"
                style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
                <h3 className="font-bold text-white mb-2 text-sm">{item.q}</h3>
                <p className="text-white/50 text-sm leading-relaxed">{item.r}</p>
              </div>
            ))}
          </div>
        </div>
