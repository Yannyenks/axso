...
// pipeline de clonage de lib/theme-import-clone.ts.
//
...
// en fiche produit). Provisionner une boutique ne fait ensuite plus aucun
// appel IA — uniquement du clonage déterministe.
...
export async function provisionerThemeDepuisLibrairie(params: {
  ...
  const { tenantId, categorie, produits, slug, nomBoutique } = params;
  const entree = selectionnerGabaritLibrairie(categorie);
